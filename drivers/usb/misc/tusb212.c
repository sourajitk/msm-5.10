...
void tusb212_set_current_path(enum usb_path path)
{
	usb_current_path = path;
}EXPORT_SYMBOL_GPL(tusb212_set_current_path);
...
int tusb212_enable(int mode)
{
	...
	switch (mode) {
		case TUSB212_DISABLE:
			if (!current_redriver_mode) {
				pr_info("%s already disable, ignore\n", __func__);
				break;
			}
		...
		case TUSB212_ENABLE:
			if (current_redriver_mode) {
				pr_info("%s already enable, ignore\n", __func__);
				break;
			}
			...
			if(dcb==0x6){ //tusb216
				pr_info("[%s] tusb216\n", __func__);
				/*
				if (usb_current_path == BTM && usb_btm_role == USB_ROLE_HOST){
					pr_info("[%s] usb_btm_role == USB_ROLE_HOST\n", __func__);
					tusb212_register_write(ACB_LVL, (acb&~(0xf0))|(0x60));
					tusb212_register_read(ACB_LVL, &acb);
					pr_info("[%s] tune ACB_LVL: 0x%x \n", __func__, acb);

					tusb212_register_write(DCB_LVL, (dcb&~(0x0f))|(0x0f));
					tusb212_register_read(DCB_LVL, &dcb);
