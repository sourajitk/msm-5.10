...
void repeater_set_usb_btm_role(enum usb_role role)
{
	usb_btm_role = role;
}EXPORT_SYMBOL_GPL(repeater_set_usb_btm_role);
...
static void eusb2_repeater_update_seq(struct eusb2_repeater *er, u32 *seq, u8 cnt)
{
	...
	dev_info(er->ur.dev, "[eusb2_repeater] dtsi param override seq count:%d\n", cnt);
	for (i = 0; i < cnt; i = i+2) {
...
static int eusb2_repeater_init(struct usb_repeater *ur)
{
	...
	if (usb_current_path == BTM && usb_btm_role == USB_ROLE_DEVICE) {
		dev_info(er->ur.dev, "[eusb2_repeater] hardcode override BTM client params\n", __func__);
		eusb2_i2c_write_reg(er, USB2_RX_CONTROL, 0x20); //0x06
		eusb2_i2c_write_reg(er, USB2_TX_CONTROL2, 0x67); //0x08
	} else if (usb_current_path == BTM && usb_btm_role == USB_ROLE_HOST) {
		dev_info(er->ur.dev, "[eusb2_repeater] hardcode override BTM host params\n", __func__);
		eusb2_i2c_write_reg(er, USB2_RX_CONTROL, 0x20); //0x06
	}
