...
extern void tusb212_set_current_path(enum usb_path path);
extern void repeater_set_usb_btm_role(enum usb_role role);
...
void rt1715_role_switch(enum usb_role role) {
	...
	repeater_set_usb_btm_role(btm_role);
	if(current_path == HUB) {
...
#endif
		if(Gamepad_active){
			dev_info(mdwc->dev, "[USB] %s Gamepad_active, ignore!\n", __func__);
			mutex_unlock(&mdwc->role_switch_mutex);
			return;
		}
		mdwc->vbus_active = false;
...
#endif
		if(Inbox_active){
			dev_info(mdwc->dev, "[USB] %s Inbox_active, ignore!\n", __func__);
			mutex_unlock(&mdwc->role_switch_mutex);
			return;
		}
		mdwc->vbus_active = false;
...
static int dwc3_msm_set_role(struct dwc3_msm *mdwc, enum usb_role role)
{
	 ...
	 */
	if (mdwc->drd_state != DRD_STATE_UNDEFINED && cur_role == role
		&& !(role == USB_ROLE_NONE && btm_role != USB_ROLE_NONE)) {
		dev_info(mdwc->dev, "[USB] %s, no USB role change\n", __func__);
...
void usb_path_switch(int val){
	...
	if (socid == AI2201_8450 && hwid == AI2201_EVB){
		...
		if(val==NONE) {//NONE
			...
			usb3803_set_mode(0);
			tusb212_enable(0);
		} else if(val==SIDE) {//SIDE (no cross hub)
			ret = gpio_direction_output(mux_gpio_ctrl->MUX1_OE, 0);
			if (ret) {
				pr_err("[USB_mux] failed to control MUX1_OE\n");
			}
			...
			usb3803_set_mode(0);
			tusb212_enable(0);
		} else if(val==BTM) {//BTM (hub mode disable port1 & port2)
			ret = gpio_direction_output(mux_gpio_ctrl->MUX1_OE, 1);
			if (ret) {
				pr_err("[USB_mux] failed to control MUX1_OE\n");
			}
			if (btm_role == USB_ROLE_DEVICE){
				pr_info("[USB] %s BTM client, redriver disable\n", __func__);
				mux2_switch(0);
				usb3803_set_mode(1);
				tusb212_enable(0);
			} else if (btm_role == USB_ROLE_HOST){
				pr_info("[USB] %s BTM host, HUB_BTM, redriver disable\n", __func__);
				mux2_switch(2);
				usb3803_set_mode(3);
				tusb212_enable(0);
			}
