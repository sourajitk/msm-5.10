                    ...
                    const char *buf, size_t count)
{
    ...
    if (ChgPD_Info.AdapterVID == 0x0b05) {
...
void monitor_charging_enable(void)
{
    ...
    if (g_Charger_mode) {
        CHG_DBG_E("%s. Charger mode battery protection monitor", __func__);
	if (bat_capacity == 100 && !g_cos_over_full_flag) {
		g_charger_mode_full_time ++;
		 CHG_DBG_E("%s. Charger mode battery g_charger_mode_full_time  : %d", __func__, g_charger_mode_full_time);
		if (g_charger_mode_full_time >= g_ultra_cos_spec_time) {
                        tmp = 1;
                        ChgPD_Info.ultra_bat_life = tmp;
               		tmp = ChgPD_Info.ultra_bat_life||ChgPD_Info.demo_app_status;
    			CHG_DBG_E("%s. set BATTMAN_OEM_Batt_Protection : %d", __func__, tmp);
