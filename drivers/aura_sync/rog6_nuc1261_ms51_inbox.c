...
static int pd_write_cmd(u16 addr, u8 *cmd)
{
	struct hid_device *hdev;
	int ret = 0;
	u8 *buffer;
	...
	buffer = kzalloc(FEATURE_WRITE_32b_COMMAND_SIZE, GFP_KERNEL);
	memset(buffer,0,FEATURE_WRITE_32b_COMMAND_SIZE);
	hdev = rog6_inbox_hidraw->hid;
	...
	hid_hw_power(hdev, PM_HINT_FULLON);
	ret = hid_hw_raw_request(hdev, NUC1261_REPORT_ID, buffer, FEATURE_WRITE_32b_COMMAND_SIZE,
					HID_FEATURE_REPORT, HID_REQ_SET_REPORT);
	...
	memset(buffer,255,FEATURE_WRITE_32b_COMMAND_SIZE);
	buffer[0] = NUC1261_REPORT_ID;
	buffer[1] = 0x2;		//I2C 32bytes WRITE CMD
	buffer[2] = 0x99;		//Reserved
	buffer[3] = 0x24;
...
static int WT6639F_firmware_check(void)
{
	u16 addr = 0;
	u16 checksum = 0;
	int i = 0, addr_offset=0;
	u8 cmd[6] = {0};
	u8 *buffer;
	int err = 0;
	int ret = 0;
	...
	do{
		...
		//else
			//printk("[ROG6_INBOX]%s-read [%x]%x, %x, %x, .... %x, %x, %x\n", __func__,addr , buffer[1], buffer[2], buffer[3], buffer[62], buffer[63], buffer[64]);
		msleep(10);
		//get sum from 64 byte buffer
		if(buffer[0] == 0x0b){	//check report id
			for(i=1; i<=read_len; i++){
				addr_offset = i-1;
				if(addr+addr_offset < 0x7FFE) {
					if(buffer[i] != 0) {
						checksum += buffer[i];
						//printk("[ROG6_INBOX] %s checksum =  0x%x, data=%x",__func__ ,checksum ,buffer[i]);
					}
				} else if (addr+addr_offset == 0x7FFE){
					printk("[ROG6_INBOX] %s checksum = 0x%x%x ",__func__ ,buffer[i] ,buffer[i+1]);
					if(((buffer[i]<<8) + buffer[i+1]) == checksum){
						printk("[ROG6_INBOX]PD firmware checksum pass!");
						ret = 1;
					} else {
						printk("[ROG6_INBOX]checksum failed! checksum should be 0x%x%x but get 0x%x",buffer[i] ,buffer[i+1] ,checksum);
					}
				} else {
					printk("[ROG6_INBOX] addr is invalid 0x%x\n",addr+addr_offset);
					break;
				}
			}
		} else {
...
int is_pd_i2c_busy(void){
	...
	err = asus_usb_hid_read_aprom(i2c_addr_select(nuc1261), cmd, NUC1261_CMD_LEN, &busy_state);
	if (err < 0){
		printk("[ROG6_INBOX][%s] asus_usb_hid_write_aprom:err %d\n", __func__, err);
	}
...
if(write_len == 4){
	//enable 32-bits data write mode
	memset(cmd,0,6);
	cmd[0] = 0x24;
	cmd[1] = 0x03;
	cmd[2] = 0x00;
	...
	err = asus_usb_hid_write_aprom(i2c_addr_select(nuc1261), cmd, 6);
	if (err < 0)
		printk("[ROG6_INBOX][%s] enable 32bits write mode:err %d\n", __func__, err);
}
	buf = kmalloc(sizeof(unsigned char)*write_len, GFP_DMA);
	if (!buf) {
	...
	do{
		//update firmware
		memcpy(buf, fw_buf+addr, write_len);
		err = pd_write_cmd(addr, buf);
		if(err < 0){
			printk("[ROG6_INBOX] %s flash PD failed",__func__);
			break;
		}
		addr += write_len;
	} while(addr < 0x8000);
...
static int humidity_correction(int humidity, int temperature)
{
	...
	raw_ambien = kzalloc(2, GFP_KERNEL);
	memset(raw_ambien, 0, 2);
	cmd[0] = 0xCF;
	...
	printk("[ROG6_INBOX]%s humidity_corrected=%d",__func__,humidity_corrected);
...
static ssize_t pd_fw_ver_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	char version[4];
	int checksum = 0;
	//checksum = WT6639F_get_fw_checksum();

	switch(checksum){
		case 0x7c18:
			strncpy(version,"V16",3);
			break;
		case 0x2465:
			strncpy(version,"V14",3);
			break;
		case 0x0f19:
			strncpy(version,"V13",3);
			break;
		case 0xe277:
			strncpy(version,"V11",3);
			break;
		case 0x0e21:
			strncpy(version,"V09",3);
			break;
		default:
			strncpy(version,"V00",3);
			break;
	}
